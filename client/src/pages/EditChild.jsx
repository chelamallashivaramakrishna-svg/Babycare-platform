...
import { UserPlus, Calendar, Activity, Droplet, ShieldAlert, Key, Monitor, ArrowLeft, Edit } from 'lucide-react';
...
const EditChild = () => {
    ...
    useEffect(() => {
        const fetchChild = async () => {
            try {
                ...
                const { data } = await axios.get(`/api/children/${childId}`, config);
                // Format date to YYYY-MM-DD for input field
                const formattedDate = data.dob ? new Date(data.dob).toISOString().split('T')[0] : '';
                setFormData({
                    name: data.name || '',
                    dob: formattedDate,
                ...
                alert("Could not load child profile. Ensure you have permission.");
                navigate('/dashboard');
            } finally {
                setLoading(false);
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await axios.put(`/api/children/${childId}`, formData, config);
            navigate('/dashboard');
        } catch (error) {
            console.error(error);
    ...
    return (
        <motion.div
            ...
            className="max-w-3xl mx-auto mt-8 mb-16 p-4 md:p-8 bg-card text-card-foreground rounded-3xl shadow-xl border border-primary/20 relative overflow-hidden"
        >
            ...
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h2 className="text-3xl font-extrabold tracking-tight text-foreground">Edit Profile: {formData.name}</h2>
                    <p className="text-muted-foreground font-medium">Update settings, login PIN, and health information.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <motion.button
                    ...
                    className="mt-8 w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold py-4 px-6 rounded-2xl shadow-lg hover:shadow-xl transition-all flex justify-center items-center gap-2 disabled:opacity-70"
                >
                    {isSubmitting ? (
                        <>
